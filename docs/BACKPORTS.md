...
The republished 1.7.0 corrects its initial slot presentation. The title font and baseline, payout labels, Bet/Spin sprites, reel markers, reel-strip offsets, downward scrolling and result messages follow the native 1.2.1 CreateReel, Draw GUI Begin, Draw GUI End and Step routines. The shared arcade fonts and Bet button were compared byte for byte with 1.2.1; the Spin button is imported from that build. The original CRT shader needs two GLES compatibility fixes: computing its uniform-dependent aspect ratio inside the function and setting boolean uniforms through integer setters. Its effect calculations remain unchanged. Device tests compare rendered title, controls, payout labels, markers and wrapped reels against these recovered drawing coordinates, and check shader compilation and output pixels. The controller footer and 4:3 screen composition remain patch adaptations.
