...
- New adventure (`src/gml/menus/setup.gml`) keeps Begin one press away and explains the highlighted choice. Challenge presets are Hero's Path, Second Quest and Master Quest. Each player's last setup is stored as plain numbers under `[Setup]` in `nova-menu.ini`, because ini values cannot hold JSON.
