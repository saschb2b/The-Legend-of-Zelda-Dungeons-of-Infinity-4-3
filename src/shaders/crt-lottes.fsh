...
// GameMaker ES adaptation: native 256x224 sampling, flat geometry,
// aperture grille, and a low-resolution beam-width adjustment.
...
void main()
{
    ...
    // A wider beam must spread the same energy, including on smaller displays.
    vec3 color = Tri(pos) * sqrt(-hardScan / 6.0) + Bloom(pos) * bloomAmount;
    // Physical output pixels keep phosphors stationary while the camera moves.
